...
If you want to use Anthropic via Vertex AI or AWS Bedrock, set `ANTHROPIC_CLIENT_TYPE` to `ANTHROPIC_VERTEX` or `ANTHROPIC_BEDROCK` and provide the relevant settings (`ANTHROPIC_VERTEX_PROJECT_ID` plus `ANTHROPIC_VERTEX_REGION`, or `ANTHROPIC_BEDROCK_REGION`).
